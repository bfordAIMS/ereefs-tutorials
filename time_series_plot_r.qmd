...
* Extracting eReefs data using the online [data extraction tool](https://extraction.ereefs.aims.gov.au/extract-data)
...
```{r figCurrents, echo=FALSE, out.width="75%", fig.align="center", fig.cap="eReefs 2016 yearly mean temperatures (left) and currents (right) for the Great Barrier Reef showing north-to-south temperature gradient and current bifurcation north-east of Cairns (shown in red and orange)."}
...
2. [Download the git repository for this tutorial]**LINK HERE**, and unzip the folder in a suitable place on your hard drive.
...
4. Open the file `tutorial__time_series.Rmd` and run the code chunk below by clicking the `r fontawesome::fa(name="play", fill = "green")` button in the top right corner. Chunks can also be run be pressing ctrl+shift+enter and sections of code (e.g. half a line, a single line, multiple lines) can be run by selecting the section and pressing ctrl+enter (replace ctrl with cmd on Mac). 
...
```{r plotSite, out.width="75%", fig.align="center", fig.cap="Interactive map showing site for which eReefs data was extracted."}
...
```{r plotTS, out.width="75%", fig.align="center", fig.cap="Time series plot of daily mean northward current velocity."}
data %>% 
...
```{r figTSOctFeb, echo=FALSE, message=FALSE, out.width="75%", fig.align="center", fig.cap="Times series plot of daily mean northward current velocity during October through January for each year 2010-11 to 2021-22."}
data %>% 
  filter(variable == "v") %>%
...
```{r figTSOct, out.width="75%", fig.align="center", fig.cap="Times series plot of daily mean northward current velocity during October for each year 2010 to 2021."}
data %>% 
  filter(variable == "v") %>%
...
```{r plotlyJanDec, out.width="85%", fig.align="center", fig.cap="Interactive plot of daily mean northward current velocity for September through August 2010-11 to 2021-22 (double click years to toggle display of all years)."}
...
```{r figWindCurrLine, out.width="75%", fig.align="center", fig.cap="Time series of daily mean northward current and wind velocities for September 2010 to September 2022."}
...
```{r figWindCurrScatter, warning=FALSE, out.width="75%", fig.align="center", fig.cap="Scatter plot of daily mean northward wind and current velocities for September 2010 to September 2022."}
