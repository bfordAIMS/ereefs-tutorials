...
**Longitude and latitude**: We want temperatures for every available longitude and latitude so we can plot the data across the entire spatial range of the eReefs model. Therefore we want to start at index 1 and count for the entire length of the two dimensions. We could note the lengths from the summary output, where we see $\texttt{longitude = 491}$ and $\texttt{latitude = 723}$. However we could also get the lengths programatically. 
...
:::{.callout-note appearance="simple"}
The dimensions of the NetCDF file have corresponding variables. Note that the dimension $\texttt{k}$ has a corresponding variable `zc` (see summary output). Therefore we would extract the dimension $\texttt{k}$ variable with `var.get.nc(..., variable = "zc")`.
:::
...
time_i <- 10   # index time = 10 --> 10th day of month
depth_i <- 16  # index k = 16 --> depth = 1.5 m

start <- c(1, 1, depth_i, time_i)  
